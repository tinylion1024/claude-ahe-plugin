...
async function runAnalyze(lastN) {
    const config = getConfig();
    ...
    console.log(`Time range: ${report.analysis_info.time_range.start || 'N/A'} to ${report.analysis_info.time_range.end || 'N/A'}`);
    console.log();
    ...
    if (report.tool_statistics.length > 0) {
        ...
        for (const tool of topTools) {
            console.log(`  ${tool.name}: ${tool.call_count} calls, ${tool.avg_time_ms.toFixed(0)}ms avg, ${tool.error_rate_percent.toFixed(1)}% errors`);
        }
        console.log();
    }
...
async function runStatus() {
    ...
    try {
        ...
        if (summaries.length > 0) {
            ...
            console.log(`  Errors: ${lastSession.statistics.error_count} (${lastSession.statistics.error_rate_percent.toFixed(1)}%)`);
        }
