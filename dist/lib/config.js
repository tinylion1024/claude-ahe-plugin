...
export const AHEConfigSchema = z.object({
    ...
    analysis: z.object({
        ...
        error_keywords: z.array(z.string()).default([
...
export function loadConfig() {
    ...
    const config = {
        collection: {
            ...
            max_trace_files: parseEnvInt(process.env.AHE_MAX_TRACE_FILES, DEFAULT_CONFIG.collection.max_trace_files),
            truncate_output_chars: parseEnvInt(process.env.AHE_TRUNCATE_CHARS, DEFAULT_CONFIG.collection.truncate_output_chars),
            trace_dir: (envTraceDir && validatePath(envTraceDir))
                ? envTraceDir
                : DEFAULT_CONFIG.collection.trace_dir,
            max_trace_age_days: parseEnvInt(process.env.AHE_MAX_TRACE_AGE_DAYS, DEFAULT_CONFIG.collection.max_trace_age_days),
        },
        analysis: {
            default_lookback_sessions: parseEnvInt(process.env.AHE_LOOKBACK_SESSIONS, DEFAULT_CONFIG.analysis.default_lookback_sessions),
            slow_operation_threshold_ms: parseEnvInt(process.env.AHE_SLOW_THRESHOLD_MS, DEFAULT_CONFIG.analysis.slow_operation_threshold_ms),
            error_keywords: DEFAULT_CONFIG.analysis.error_keywords, // Keep default keywords
            analysis_dir: (envAnalysisDir && validatePath(envAnalysisDir))
                ? envAnalysisDir
                : DEFAULT_CONFIG.analysis.analysis_dir,
        },
        display: {
            show_timestamps: parseEnvBool(process.env.AHE_SHOW_TIMESTAMPS, DEFAULT_CONFIG.display.show_timestamps),
            show_execution_times: parseEnvBool(process.env.AHE_SHOW_TIMES, DEFAULT_CONFIG.display.show_execution_times),
            max_issues_shown: parseEnvInt(process.env.AHE_MAX_ISSUES, DEFAULT_CONFIG.display.max_issues_shown),
        },
    };
