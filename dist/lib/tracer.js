 ...
 * @module lib/tracer
 */
import { appendFileSync, existsSync, readFileSync, readdirSync, statSync, unlinkSync, writeFileSync } from 'fs';
...
export class TraceManager {
     ...
     */
    saveTrace(toolName, toolInput, toolOutput, executionTimeMs, sessionId, workingDirectory, truncateLength) {
        const config = getConfig();
     ...
     */
    loadTraces(sessionId, lastN, since) {
        const traces = [];
        if (sessionId) {
     ...
     */
    cleanupOldTraces(maxFiles, maxAgeDays) {
        const config = getConfig();
